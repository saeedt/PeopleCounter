...
unsigned long NewPing::ping_cm_tc(float temp, unsigned int max_cm_distance) {
	unsigned long echoTime = NewPing::ping(max_cm_distance); // Calls the ping method and returns with the ping echo distance in uS.
	float RoundTrip_CM = 20000/(331.3 + 0.606 * temp);
#if ROUNDING_ENABLED == false	
	return (echoTime / RoundTrip_CM);	
#else
	return NewPingConvert(echoTime, (unsigned int)RoundTrip_CM); // Convert uS to centimeters.
#endif
}


unsigned long NewPing::ping_in_tc(float temp, unsigned int max_cm_distance) {
	unsigned long echoTime = NewPing::ping(max_cm_distance); // Calls the ping method and returns with the ping echo distance in uS.
	float RoundTrip_IN = 50800/(331.3 + 0.606 * temp);
#if ROUNDING_ENABLED == false
	return (echoTime / US_ROUNDTRIP_IN);              // Call the ping method and returns the distance in inches (no rounding).
#else
	return NewPingConvert(echoTime, (unsigned int)US_ROUNDTRIP_IN); // Convert uS to inches.
#endif
}
